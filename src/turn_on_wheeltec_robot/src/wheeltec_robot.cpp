...
auto createQuaternionMsgFromYawCustom(double yaw)
{
	tf2::Quaternion q;
	q.setRPY(0, 0, yaw);
	return tf2::toMsg(q);
...
void turn_on_robot::Publish_Odom()
{
    ...
    //把Z轴转角转换为四元数进行表达
    geometry_msgs::msg::Quaternion odom_quat = createQuaternionMsgFromYawCustom(Robot_Pos.Z);
...
功能: 构造函数, 只执行一次，用于初始化
***************************************/
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0),Node("wheeltec_robot_node"), _Now(0, 0, RCL_ROS_TIME), _Last_Time(0, 0, RCL_ROS_TIME)
{
  //Clear the data
  //清空数据
  ...
  std::string default_usart_port_name = "/dev/wheeltec_controller";
  int default_serial_baud_rate = 115200;
  std::string default_odom_frame_id = "odom_combined";
  std::string default_robot_frame_id = "base_footprint";
  std::string default_gyro_frame_id = "gyro_link";

  this->declare_parameter("usart_port_name", default_usart_port_name);
  this->declare_parameter("serial_baud_rate", default_serial_baud_rate);
  this->declare_parameter("odom_frame_id", default_odom_frame_id);
  this->declare_parameter("robot_frame_id", default_robot_frame_id);
  this->declare_parameter("gyro_frame_id", default_gyro_frame_id);

  this->get_parameter_or("usart_port_name", usart_port_name, default_usart_port_name);
  this->get_parameter_or("serial_baud_rate", serial_baud_rate, default_serial_baud_rate);
  this->get_parameter_or("odom_frame_id", odom_frame_id, default_odom_frame_id);
  this->get_parameter_or("robot_frame_id", robot_frame_id, default_robot_frame_id);
  this->get_parameter_or("gyro_frame_id", gyro_frame_id, default_gyro_frame_id);

  // private_nh.param<std::string>("usart_port_name",  usart_port_name,  default_usart_port_name); //Fixed serial port number //固定串口号
  // private_nh.param<int>        ("serial_baud_rate", serial_baud_rate, default_serial_baud_rate); //Communicate baud rate 115200 to the lower machine //和下位机通信波特率115200
  // private_nh.param<std::string>("odom_frame_id",    odom_frame_id,    default_odom_frame_id);      //The odometer topic corresponds to the parent TF coordinate //里程计话题对应父TF坐标
  // private_nh.param<std::string>("robot_frame_id",   robot_frame_id,   default_robot_frame_id); //The odometer topic corresponds to sub-TF coordinates //里程计话题对应子TF坐标
  ...
  timer_ = this->create_wall_timer(10ms, std::bind(&turn_on_robot::Control, this));
